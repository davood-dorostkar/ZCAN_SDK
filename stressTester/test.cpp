...
IGNORE_TEST(sdk_send_receive, 04_receive)
{
    ...
    CHECK_EQUAL(STATUS_OK, ZCAN_Receive(chHandle, &receiveFrame, 8, 10));
}
...
TEST(custom_transmission, 01_init)
{
    ZCAN_SetValue(CAN, "0/baud_rate", "500000");
    CHANNEL_HANDLE chHandle = zcan.InitCan(CAN);
    CHECK_EQUAL(CAN, chHandle);
}

TEST(custom_transmission, 02_send_custom_packet)
{
    ZCAN_SetValue(CAN, "0/baud_rate", "500000");
    CHANNEL_HANDLE chHandle = zcan.InitCan(CAN);
    ZCAN_StartCAN(chHandle);
...
TEST(custom_transmission, 03_send_custom_packet)
{
    ZCAN_SetValue(CAN, "0/baud_rate", "500000");
    CHANNEL_HANDLE chHandle = zcan.InitCan(CAN);
    ZCAN_StartCAN(chHandle);
    BYTE customData[8] = { 0xF1, 0xF2, 0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8 };
    CHECK_EQUAL(8, zcan.SendMessage(CAN, customData));
}